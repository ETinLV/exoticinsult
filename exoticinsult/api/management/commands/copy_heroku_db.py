...
class Command(BaseCommand):
    ...
    def create_db(self):
        proc = subprocess.call("psql -c 'CREATE DATABASE {} OWNER {};'".format(self.db_name, self.username),
                               shell=True, cwd=BASE_DIR)
        return proc

    def restore_db(self):
        process = subprocess.Popen(
            "pg_restore --verbose --clean --no-acl --no-owner -h localhost -U {} -d {} latest.dump".format(
                self.username, self.db_name).split(), stdout=subprocess.PIPE, cwd=BASE_DIR)
        output, error = process.communicate()
        return output
